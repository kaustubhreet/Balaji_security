...
const PositionCommodityAd = () => {
  ...
  useEffect(() => {
    if (email) {
      ...
      axios.get(`${backendUrl}/positionHoldingApi/positionCommodity/${email}`).then((response) => {
  ...
  const handleDelete = (id) => {
    axios.delete(`${backendUrl}/positionHoldingApi/positionCommodity/${id}`)
      .then(() => {
  ...
  const handleUpdate = (id, updatedRow) => {
    axios.put(`${backendUrl}/positionHoldingApi/positionCommodity/${id}`, updatedRow).then(() => {
  ...
  const handleAdd = (newRow) => {
    axios.post(`${backendUrl}/positionHoldingApi/positionCommodity/${email}`, newRow)
      .then(() => {
  ...
  return (
    <Box sx={{ display: 'flex' }}>
      ...
      <Box sx={{ flexGrow: 1, p: 3, mt: '64px' }}>
        <h1 style={{ marginBlock: "30px", alignItems: "center" }}>POSITIONS COMMODITY </h1>
...
export default PositionCommodityAd;