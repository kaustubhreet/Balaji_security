...
const HoldingCommodity = () => {
  ...
  useEffect(() => {
    if (email) {
      ...
      axios.get(`${backendUrl}/positionHoldingApi/holdingCommodity/${email}`).then((response) => {
  ...
  const handleDelete = (id) => {
    axios.delete(`${backendUrl}/positionHoldingApi/holdingCommodity/${id}`)
      .then(() => {
  ...
  const handleUpdate = (id, updatedRow) => {
    axios.put(`${backendUrl}/positionHoldingApi/holdingCommodity/${id}`, updatedRow).then(() => {
  ...
  const handleAdd = (newRow) => {
    axios.post(`${backendUrl}/positionHoldingApi/holdingCommodity/${email}`, newRow)
      .then(() => {
  ...
  return (
    ...
    <Box sx={{ flexGrow: 1, p: 3, mt: '64px' }} style={{overflowX:"visible"}}>
      <h1 style={{marginBlock:"30px",alignItems:"center"}}>HOLDING COMMODITY </h1>
...
export default HoldingCommodity